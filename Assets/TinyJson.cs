...
namespace Pathfinding.Serialization {
	...
	/** Indicates that the full type of the instance will always be serialized. This allows inheritance to work properly. */
		...
		string EatUntil (string c, bool inString) {
			...
			while (true) {
				...
				if (readInt == -1) {
					...
					if (reader.Peek() == 'u') {
						reader.Read();
						string u = "" + (char)reader.Read() + (char)reader.Read() + (char)reader.Read() + (char)reader.Read();
						builder.Append((char)ushort.Parse(u, NumberStyles.AllowHexSpecifier));
						escape = false;
					}
